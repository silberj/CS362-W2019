...
bool randomTest1 = false, randomTest2 = false, randomTest3 = false, randomTest4 = false;
int failedTestCriteria3 = 0;
int failedTest1 = 0, failedTest2 = 0, failedTest3 = 0, failedTest4 = 0;
int successTest1 = 0, successTest2 = 0, successTest3 = 0, successTest4 = 0;
...
void checkVillageTest(int testNumber, int currentPlayer, struct gameState *postState) 
{
	...
	cardEffect(village, choice1, choice2, choice3, postState, handpos, &bonus);
	...
	if(testNumber == 0)
	{
		...
		if(preState.deckCount[currentPlayer] == 0)
		{
			...
			{
				printf("TEST 1.2.1 - Initial deckCount = %d, deckCount = %d, expected result = %d\n", preState.deckCount[currentPlayer], postState->deckCount[currentPlayer], preState.discardCount[currentPlayer] - drawnCards);
				failedTest1++;
			}			
			else
				successTest1++;			
	...
	{	
		...
		if(assertTrue(preState.numActions + newNumberOfActions, postState->numActions) == false)	
		{
			printf("TEST 2.1 - Initial numActions = %d, expected = %d, received = %d\n", preState.numActions, preState.numActions + newNumberOfActions, postState->numActions);
			failedTest2++;
		}
		else
			successTest2++;
	...
	{	
		...
		{		
			printf("-- TEST 3: Test that input handPos card is discarded and replaced with card from back. --\n");
			randomTest3 = true;			
		}
		...
		if(preState.deckCount[currentPlayer] > drawnCards)
		{
			if(assertTrue(postState->handCount[currentPlayer], preState.handCount[currentPlayer] + drawnCards - 1) == false)
			{
				printf("TEST 3.1 - Initial handCount = %d, handCount = %d, expected = %d\n", preState.handCount[currentPlayer], postState->handCount[currentPlayer], preState.handCount[currentPlayer] + drawnCards - 1);
				failedTest3++;
			}
			else
				successTest3++;
	...
	// ----------- TEST 4: Test that the input handPos card is not trashed. --------------
	else if(testNumber == 3)
	{	
		if(randomTest4 == false)
		{			
			printf("-- TEST 4: Test that the input handPos card is not trashed. --\n");
			randomTest4 = true;			
		}
	
		if(assertTrue(postState->playedCardCount, preState.playedCardCount + 1) == false)
		{
			printf("TEST 4.1 - TestState.handplayedCardCount = %d, expected = %d\n", postState->playedCardCount, preState.playedCardCount + 1);
			failedTest4++;
		}
		else
			successTest4++;
...
int main () 
{
	...
	for (testNumber = 0; testNumber < 4; testNumber++)
	{
		for (testRun = 0; testRun < 20; testRun++)
		{
			...
			Testing.numActions = randomNumber(MIN_DECK, MAX_DECK);
			...
			checkVillageTest(testNumber, player, &Testing);
		}
	}
